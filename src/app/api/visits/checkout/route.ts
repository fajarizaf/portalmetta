...
export async function POST(request: NextRequest) {
  ...
  if (rec.qrStatus === "pending") {
    await logScanActivity({
      recordId: rec.id,
      token,
      action: "checkout",
      success: false,
      error: "Visitor belum check-in",
      ip,
      scannedBy: me.email,
    })
    return NextResponse.json({ error: "Visitor belum check-in" }, { status: 400 })
  }

  if (rec.qrStatus === "checked_out") {
    await logScanActivity({
      recordId: rec.id,
      token,
      action: "checkout",
      success: false,
      error: "Visitor sudah check-out",
      ip,
      scannedBy: me.email,
    })
